...
class MakeManyMasksWrapper:
    ...
    def __init__(self, impl, variants_n=2):
        self.impl = impl#一个实现函数或对象，用于生成掩码
        self.variants_n = variants_n#生成掩码的数量，默认为2

    def get_masks(self, img):
        #该方法接受一张图像 img 并生成多个掩码
        img = np.transpose(np.array(img), (2, 0, 1))#首先将图像转换为NumPy数组并转置，使其维度顺序变为 (通道数, 高度, 宽度)
        return [self.impl(img)[0] for _ in range(self.variants_n)]#使用 self.impl 函数生成多个掩码，每次调用生成一个掩码，共生成 self.variants_n 个掩码

def process_images(src_images, indir, outdir, config):#对图像进行处理，并生成相应的掩码图像
#indir: 输入目录路径，outdir: 输出目录路径，config: 配置对象，包含生成掩码和图像处理的相关参数
    ...
    for infile in src_images:#源图像文件的列表
        try:
            file_relpath = infile[len(indir):]#获取图像的相对路径并创建输出目录
            img_outpath = os.path.join(outdir, file_relpath)
            os.makedirs(os.path.dirname(img_outpath), exist_ok=True)
            image = Image.open(infile).convert('RGB')#打开图像并转换为 RGB 模式
            #根据配置缩放图像，如果图像太小，则处理
            if min(image.size) < config.cropping.out_min_size:
                ...
                out_size = (np.array(image.size) * factor).round().astype('uint32')
                image = image.resize(out_size, resample=Image.BICUBIC)
            src_masks = mask_generator.get_masks(image)#生成掩码
            #根据配置过滤和裁剪掩码和图像
...
def _list_image_files_recursively(data_dir):#递归地列出给定目录及其子目录中的所有图像文件
    results = []
    for entry in sorted(bf.listdir(data_dir)):
        full_path = bf.join(data_dir, entry)
        ext = entry.split(".")[-1]
        if "." in entry and ext.lower() in ["jpg", "jpeg", "png","bmp", "gif"]:
            results.append(full_path)
        elif bf.isdir(full_path):
            results.extend(_list_image_files_recursively(full_path))
    return results

def main(args):#处理图像并生成掩码
    print("Generating masks for images in", args.indir)
    if not args.indir.endswith('/'):
        args.indir += '/'
    os.makedirs(args.outdir, exist_ok=True)
    config = load_yaml(args.config)
    in_files = _list_image_files_recursively(args.indir)
    if args.n_jobs == 0:
