...
def make_batch(image_path, mask_path, device):
    ...
    masked_image = (1 - mask) * image  # Calculate masked_image
    ...
    with torch.no_grad():
        with model.ema_scope():
            for image_path, mask_path in tqdm(zip(images, masks)):
                ...
                # Save masked_image
                masked_image = (batch["masked_image"] + 1.0) / 2.0  # Scale masked_image back to [0,1]
                masked_image = torch.clamp(masked_image, min=0.0, max=1.0).cpu().numpy().transpose(0, 2, 3, 1)[0] * 255
                masked_outpath = os.path.join(opt.outdir, 'masked_' + os.path.split(image_path)[1])
                Image.fromarray(masked_image.astype(np.uint8)).save(masked_outpath)