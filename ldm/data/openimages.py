...
try:
    from ldm.data.openimages_bbox_helpers.openimages_bbox_base import OpenImagesBBoxBase
...
class FullOpenImagesBase(Dataset):
    def __init__(self, size=None, crop_size=None, interpolation="bicubic",
        ...
        self.image_rescaler = albumentations.SmallestMaxSize(
            max_size=self.size if self.size is not None else self.crop_size,
        ...
        self.unident_files = "unidentifiable_openimage_files.txt"
    ...
    def __getitem__(self, i):
        ...
        if self.size is not None or (
                self.crop_size is not None and min(image.shape[0], image.shape[1]) < self.crop_size):
...
class FullOpenImagesTrain(FullOpenImagesBase):
    def __init__(self, size=None, crop_size=None, random_crop=True, interpolation="bicubic"):
        super().__init__(size=size,crop_size=crop_size,random_crop=random_crop,interpolation=interpolation)
...
class SemanticOpenImages(Dataset):
    ...
    def __init__(self, csv_root='data/semantic_openimages', image_root='data/openimages/', size=None, crop_size=None,
        ...
        self.data_csv = pd.read_csv(
            os.path.join(self.csv_root, f'{self.split}-annotations-object-segmentation.csv')).to_dict()
        self.size = size
        ...
        self.labels = {
            ...
            'segmentation_path_': [os.path.join(self.segmentation_root, fn) for fn in
                                   self.data_csv['MaskPath'].values()]
        }
...
class OpenImagesBBoxTrain(OpenImagesBBoxBase):
    ...
    def __init__(self, size, encode_bbox=True, datapath='data/openimages', n_max_samples=-1,
        ...
        self.size = size
        super().__init__(data_path=datapath,
                         split='train',
                         keys=keys,
                         target_image_size=size,
                         no_max_samples=n_max_samples,
                         crop_method=crop_type,
                         random_flip=random_flip,
                         encode_crop=encode_bbox,
                         **kwargs)
