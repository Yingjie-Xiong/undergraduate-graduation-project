...
class DDPM(pl.LightningModule):
    def __init__(self,
                 unet_config,
                 timesteps=1000,#扩散的步数
                 beta_schedule="linear",
                 loss_type="l2",
                 ckpt_path=None,
                 ignore_keys=[],
                 load_only_unet=False,
                 monitor="val/loss",#用于监控的指标
                 use_ema=True,
                 first_stage_key="image",
                 image_size=256,
                 channels=3,
                 log_every_t=100,#每隔多少步记录一次日志
                 clip_denoised=True,
                 linear_start=1e-4,#线性调度的起始值
                 linear_end=2e-2,#线性调度的结束值
                 cosine_s=8e-3,#余弦调度的参数
                 given_betas=None,#预定义的Beta值
                 original_elbo_weight=0.,#原始ELBO的权重
                 v_posterior=0.,  # 用于选择后验方差的权重
                 l_simple_weight=1.,#简单损失的权重
                 conditioning_key=None,
                 parameterization="eps",  # 参数化方式
                 scheduler_config=None,#调度器配置
...
class LatentDiffusion(DDPM):
    def __init__(self,
                 first_stage_config,
                 cond_stage_config,
                 num_timesteps_cond=None,#1，条件阶段的时间步数
                 cond_stage_key="image",#masked_image，条件阶段的键
                 cond_stage_trainable=False,
                 concat_mode=True,#是否使用拼接模式
                 cond_stage_forward=None,#条件阶段的前向函数
                 conditioning_key=None,#条件键
                 scale_factor=1.0,#缩放因子
                 scale_by_std=False,#是否按标准偏差缩放
                 *args,
                 **kwargs):
        ...
        try:
            ...
            self.num_downs = 0 #计算第一个阶段的降采样次数
        if not scale_by_std:
        ...
        self.clip_denoised = False  #是否裁剪去噪后的图像， by default, ldm should not clip denoised since the computing is done in the latent space, and the latent space is not between -1 and 1
        self.bbox_tokenizer = None  #边界框分词器
...
#     """
#         can either run as pure inpainting model (only concat mode) or with mixed conditionings,e.g. mask as concat and text via cross-attn.
#     """
#     def __init__(
#         self,
...
class LatentInpaintDiffusion(LatentDiffusion):
#继承自LatentDiffusion类，并扩展了其功能以支持图像的修复。通过在图像输入中添加遮罩通道，能够以修复模式运行,还包括加载和微调模型参数的功能，使得模型可以从现有检查点进行初始化和优化
    """
    ...
    def __init__(self,
                 ...
                 finetune_keys_to_retain=("model.diffusion_model.input_blocks.0.0.weight",
                                          "model_ema.diffusion_modelinput_blocks00weight"),
                 concat_keys=("masked_image", "mask"),  # 用于推理时连接的键（例如，遮罩图像和遮罩）
                 masked_image_key="masked_image",#遮罩图像的键
                 keep_finetune_dims=7,#保留的微调维度数
                 c_concat_log_start=0,  # 用于记录重建的 c_concat 代码
                 c_concat_log_end=3, #用于记录重建的 c_concat 代码的结束位置
                 *args, **kwargs
                 ):
        ckpt_path = kwargs.pop("ckpt_path", None)#检查点路径，用于从现有模型中初始化参数
        ignore_keys = kwargs.pop("ignore_keys", list())#在加载检查点时忽略的键
        super().__init__(*args, **kwargs)
        self.masked_image_key = masked_image_key
        assert self.masked_image_key in concat_keys
    ...
    def init_from_ckpt(self, path, ignore_keys=list(), only_model=False):
#从检查点文件中初始化模型参数，加载检查点文件，删除指定的忽略键，保留和微调指定的参数键，然后将状态字典加载到模型中
        sd = torch.load(path, map_location="cpu")
        if "state_dict" in list(sd.keys()):
            sd = sd["state_dict"]
        #删除指定的忽略键
...
#获取输入批次数据,batch输入批次数据,bs批次大小
        ...
        for ck in self.concat_keys:
            ...
            if ck != self.masked_image_key:
                ...
                cc = self.get_first_stage_encoding(self.encode_first_stage(cc))
            c_cat.append(cc)
        c_cat = torch.cat(c_cat, dim=1)#将所有concat_keys拼接在一起
        # 无交叉注意力，#此版本消除了交叉注意力，因为在这种情况下我们不需要它（我们不以文本为条件）
        all_conds = [c_cat]
        if return_first_stage_outputs:
            return z, all_conds, x, xrec, xc
        return z, all_conds
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=50, ddim_eta=0,
                   ...
                   **kwargs):
#记录图像数据，用于可视化,batch输入批次数据,N是记录的图像数量,n_row每行图像数量,sample是否生成样本,plot_denoise_rows 和 plot_diffusion_rows是否绘制去噪和扩散行
        ema_scope = self.ema_scope if use_ema_scope else contextlib.suppress
        use_ddim = ddim_steps is not None
        log = dict()
        z, c, x, xrec, xc = self.get_input(batch, self.first_stage_key, bs=N, return_first_stage_outputs=True)
        ...
        if not (self.c_concat_log_start is None and self.c_concat_log_end is None):
            log["c_concat_decoded"] = self.decode_first_stage(c_cat[:, self.c_concat_log_start:self.c_concat_log_end])
        #记录扩散行： 如果plot_diffusion_rows为True，则记录扩散过程中的图像
        if plot_diffusion_rows:
            diffusion_row = list()
            z_start = z[:n_row]
            for t in range(self.num_timesteps):
            ...
            diffusion_grid = make_grid(diffusion_grid, nrow=diffusion_row.shape[0])
            log["diffusion_row"] = diffusion_grid
        #记录去噪行： 如果sample为True，则记录去噪过程中的图像
        if sample:
            with ema_scope("Sampling"):
